...
feature "coda transactions", :vcr do
  let(:chain) { create :coda_chain }

  before do
    visit coda_chain_transactions_path(chain)
  end
  ...
  it "displays transaction search" do
    ...
    expect(page).to have_link "Summary", href: coda_chain_path(chain)
    ...
    it "redirects to the transaction page" do
      within ".transactions-search-results" do
        within page.find("tbody > tr:first-child") do
          click_on "details"
          expect(page.current_path).to match "/coda/chains/#{chain.slug}/transactions/FEDfM8osehRwK69uz6NirULPWGe9bNsongwH"
        end
      end
    end
  end
  ...
  context "bad search" do
    before do
      visit coda_chain_transactions_path(chain, start_time: "invalid")
    end
