...
describe Polkadot::ValidatorSummary do
  describe '#indexing_completed?' do
    subject { validator_summary.indexing_completed?(last_indexed_timestamp) }

    let(:validator_summary) { described_class.new(attributes) }
    let(:attributes) do
