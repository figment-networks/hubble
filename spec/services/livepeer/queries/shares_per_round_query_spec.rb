...
RSpec.describe Livepeer::Queries::SharesPerRoundQuery, livepeer: :factory do
  ...
  it 'returns shares grouped by round and delegator' do
    ...
    expect(results[1].delegator_address).to eq(delegators[0])
    expect(results[1].fees).to eq(30)
    expect(results[1].reward_tokens).to eq(300)

    expect(results[2].round_number).to eq(rounds[1].number)
    expect(results[2].delegator_address).to eq(delegators[1])
    expect(results[2].fees).to eq(40)
    expect(results[2].reward_tokens).to eq(400)
  end
