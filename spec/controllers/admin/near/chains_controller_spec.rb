...
RSpec.describe Admin::Near::ChainsController do
  ...
  describe "POST #create" do
    context "with valid params" do
      let(:params) { attributes_for(:near_chain, slug: "nearslug2") }
      
      it "saves the new chain" do
        expect{ post :create, params: { near_chain: params } }.to change(chain.class, :count).by(1)
      end

      it "responds with 302 status" do
        post :create, params: { near_chain: params }
    ...
    context "with invalid params" do
      let(:params) { attributes_for(:near_chain, slug: chain.slug) }

      it "does not save the new chain" do
        expect{ post :create, params: { near_chain: params } }.to change(chain.class, :count).by(0)
      end
  ...
  describe "PUT #update" do 
    context "with no errors" do
      ...
      before(:each) { put :update, params: { id: chain.slug, near_chain: new_params } }
      ...
      it "redirects to admin_root_path" do
        expect(response).to redirect_to(admin_near_chain_path(chain))
      end

      it "flashes success message" do
        expect(flash[:success]).to match("Chain info has been updated!")
      end
    end
