class GovProposalDepositModal {
  constructor(el) {
    ...
    this.MEMO = 'Submit, deposit and vote on proposals with Hubble - https://hubble.figment.io';
    ...
    const triggeredGAEvent = false;
    this.modal.on('shown.bs.modal', async () => {
      this.reset();

      if (!triggeredGAEvent) {
        ga('send', 'event', 'gov-proposal-deposit', 'started');
      }
...
App.Cosmoslike.GovProposalDepositModal = GovProposalDepositModal;
