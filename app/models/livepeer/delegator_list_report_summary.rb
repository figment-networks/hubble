class Livepeer::DelegatorListReportSummary
  ...
  def data
    delegator_list.addresses.map do |delegator_address|
      FIELDS.each_with_object({}) do |field, hash|
        hash[field] = send(field)[delegator_address] || DEFAULT_VALUE
      end
    end
  end
  ...
  def fees
    @fees ||= group_by_delegator(shares, :fees)
  end

  def reward_tokens
    @reward_tokens ||= group_by_delegator(shares, :reward_tokens)
  end

  def pending_stake
    @pending_stake ||= group_by_delegator(stakes, :pending_stake)
  end

  def unclaimed_stake
    @unclaimed_stake ||= group_by_delegator(stakes, :unclaimed_stake)
  end

  def unbonding_tokens
    @unbonding_tokens ||= group_by_delegator(unbondings, :unbonding_tokens)
  end

  def unbonded_tokens
    @unbonded_tokens ||= group_by_delegator(unbonds, :unbonded_tokens)
  end
